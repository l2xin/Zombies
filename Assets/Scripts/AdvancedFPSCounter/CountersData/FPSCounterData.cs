	...
	public class FPSCounterData: BaseCounterData
	{
		...
		internal override void UpdateValue(bool force)
		{
			...
			if (dirty && main.OperationMode == OperationMode.Normal)
			{
				...
				text.Length = 0;
				text.Append(coloredStartText).Append(LastValue);
                text.Append(FPS_TEXT_END);
                ...
                text.Append("PING: " + FightManager.m_pingTime * 1000);
                text.Append(AFPSCounter.NEW_LINE);
                text.Append("Version: " + VersionManager.localVersion);
                text.Append(AFPSCounter.NEW_LINE);
                text.Append("Cache: " + PoolUtil.GetCacheParticlesCount());
                text.Append(AFPSCounter.NEW_LINE);
                if (FightManager.controlsScriptList != null)
                {
                    text.Append("RoomPCU: " + FightManager.controlsScriptList.Count);
                }
                else
                {
                    text.Append("RoomPCU: null");
                }
            }
		}
